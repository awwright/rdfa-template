...
var templateFilename = __dirname + '/data-table-about.html';
...
describe("0004", function(){
    ...
    it("genreateDocument: hyperlinks", function(){
        ...
        assert.equal(eTr[0].childNodes[0].firstChild.textContent, 'URI');
        assert.equal(eTr[1].childNodes[1].firstChild.textContent, 'http://example.com/#a');
        assert.equal(eTr[0].childNodes[1].firstChild.textContent, 'Name');
        assert.equal(eTr[1].childNodes[3].firstChild.textContent, 'Alice');
    });
});
